...
describe("CoursesSection", () => {
  it("displays all courses when the 'Todos' filter is active (default)", () => {
    render(
      <CoursesSection courses={mockCourses} categories={mockCategories} />,
    );

    expect(
      screen.getByText("Terapia Cognitivo Conductual"),
    ).toBeInTheDocument();
  ...
  it("shows only the selected category's courses after clicking a category pill", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.queryByText("Liderazgo Organizacional"),
  ...
  it("shows an empty-state message when no courses match the selected category", async () => {
    ...
    await waitFor(
      () =>
        expect(
          screen.getByText(/No hay cursos en esta categoría/i),
        ).toBeInTheDocument(),
      { timeout: 500 },
    );
  });
