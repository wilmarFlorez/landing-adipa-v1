...
export default function FormField({ id, label, state, error, children }: Props) {
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={id} className="font-heading text-sm font-semibold text-dark dark:text-darkText">
        {label}
      </label>
      {children}
      {state === "error" && error && (
        <span id={errorId} role="alert" className="font-body text-xs text-red-500 dark:text-red-400">
          {error}
        </span>
      )}
    </div>
